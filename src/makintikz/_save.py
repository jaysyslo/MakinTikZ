...
def _recurse(data: TikzData, obj: Artist) -> list:
    ...
    # Get true subplot indices
    def _axes_geom_index(ax: Axes) -> int:
        """Grid position (0-based) for an Axes with a subplotspec; -1 otherwise."""
        subplotspec = ax.get_subplotspec()
        if subplotspec is None:
            return -1
        return subplotspec.get_topmost_subplotspec().get_geometry()[2]

    # Children must be ordered by subplot index
    # Otherwise, Tikz code will be generated by Python code
    # appearance instead of matplib's subplot index order.
